...
class generate_quantum_heom():
    ...
    def generate_molecular_system(self):
        ...
        self.system_output = system.system_operators(Single_El_Int,Double_El_Int,Nel,N_qu_vib_modes,El_Nuclear_Couplings_cl,
                                        max_occ_qu_vib_modes,dim_rho,El_Nuclear_Couplings_qu,Vib_Freq_qu)

        self.d_ops,self.d,self.ddag,self.Fock_states,self.molham_func,self.molham_log,\
        self.d_ops_log,self.rho_0_log,self.identity_dim_rho,self.d_ops_dressed,\
        self.d_ops_dressed_log,self.Ham,self.Ham_log = self.system_output[0:13]
        if bool(N_qu_vib_modes):
            self.b_ops,self.b,self.bdag = self.system_output[13:16]
            if small_polaron_yn:
                self.d_ops_comp = self.d_ops_dressed
                self.d_ops_comp_log = self.d_ops_dressed_log
            else:
                self.d_ops_comp = self.d_ops
                self.d_ops_comp_log = self.d_ops
    ...
    def generate_heom_in_sparse_representation_for_example_x(self):
        ...
        x_vec_tester = np.array([1],dtype=float)
        sparsity_object = generating_sparsity_class.sparsity_heom_liouvillian(ksiglm=self.KsigLm,
            ...
            d_ops_comp=self.d_ops_comp,d_ops_comp_log=self.d_ops_comp_log,ham_log=self.Ham_log,
            ...
            molham_one_x=self.Ham,el_lead_couplings_one_x=el_lead_couplings_func(Nleads,Nel,V_Km,x_vec_tester))
    ...
    def return_sparse_heom_ingredients(self):
        ...
        self.trace_cols,self.rhs_vector,self.sparse_trace_array,self.tier_ind,self.ksiglm,self.un_ind) = sparse_heom_ingredients[0:39]

        return sparse_heom_ingredients

    def return_sparse_heom_one_x(self,ham_this_x,el_lead_couplings_this_x):

        pair_values_one_x = generate_heom_one_x.heom_liouvillian_one_x(pair_values_gamma=self.pair_values_gamma_fil,
